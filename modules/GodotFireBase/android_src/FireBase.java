...
public class FireBase extends Godot.SingletonBase {
	...
	public FireBase(Activity p_activity) {
		registerClass ("FireBase", new String[] {
			...
			//Analytics++
			"sendAchievement", "send_custom", "send_events", "join_group", "level_up", 
			"app_open", "post_score", "content_select", "earn_currency", "purchase", "set_extras",
	...
	public void set_extras(final String affiliation, final int current_level, final int coins) {
		activity.runOnUiThread(new Runnable() {
			public void run() {
				Analytics.getInstance(activity).set_extras(affiliation, current_level, coins);
			}
		});
	}

	public void app_open(final int sessions, final int sessions_after_update) {
		activity.runOnUiThread(new Runnable() {
			public void run() {
				Analytics.getInstance(activity).send_app_open(sessions, sessions_after_update);
			}
		});
	}
	...
	public void purchase(final String sku, final int amount) {
		activity.runOnUiThread(new Runnable() {
			public void run() {
				Analytics.getInstance(activity).send_purchase(sku, amount);
			}
		});
	}
